import os
import boto3
from flask import request, jsonify, json, render_template_string
...
CORS(app, resources={'*': {'origins': os.getenv('ALLOWED_ORIGINS', 'localhost:8000').split(',')}})
...
def send_number_email():
    ...
    email_body = render_template_string('Hello {{ email }} your generated number was {{ num }}', email=to_email, num=num)

    mail = Mail(from_email=os.environ['FROM_EMAIL'],
                to_emails=[to_email],
                subject='Flask SendGrid Demo: Random Number Generated',
                plain_text_content=email_body)

    mail.add_custom_arg(CustomArg(key='custom_id',
